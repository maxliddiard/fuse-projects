...
const EXPLORATION_SYSTEM = `You are a CRM analyst. Given conversations with a sales/vendor organization, extract structured insights about the commercial relationship.

Respond with ONLY valid JSON:
{
  "contacts": [{"name": "...", "identifier": "...", "role": "..."}],
...
interface SalesInsights {
  contacts: Array<{ name: string; identifier: string; role: string }>;
  dealStage: string;
...
export class SalesExplorationService {
  static async exploreSalesAccounts(emailAccountId: string): Promise<number> {
    ...
    for (let i = 0; i < salesAccounts.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map((account) => this.exploreOneEmail(account.id, emailAccountId, account.domain!)),
      );
  ...
  static async exploreWhatsAppSalesAccounts(whatsAppAccountId: string): Promise<number> {
    const salesAccounts = await prisma.discoveredAccount.findMany({
      where: {
        whatsAppAccountId,
        category: "SALES",
  ...
  private static async exploreOneEmail(
    accountId: string,
...
---`;
  ...
  private static async exploreOneWhatsApp(
    ...
    const messageContext = recentMessages
      .map((m) => {
        const bodyText = m.textBody
          ? m.textBody.substring(0, BODY_TRUNCATE_LENGTH)
          : `(${m.messageType} message)`;
        return `From: ${m.fromName || m.fromPhone} [${m.direction}]
